...
export class DetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activeRouter.paramMap.subscribe((route: any) =>{
      ...
      this.sub4 = this.weather.getCurrentWind(this.city).subscribe((windspedd)=>{this.wind = windspedd})
      this.sub5 = this.weather.getForecast(this.city).subscribe((data: any)=>{
        ...
        for(let i = 0; i < data.length; i++){
          ...
          if(((date === todayNumberInWeek + 1) || (todayNumberInWeek === 6 && date === 0)) && !this.day1Name){
            ...
            this.day3Temp = Math.round(data[i].weather[0].main.temp);
            ...
            this.day5Temp = Math.round(data[i].weather[0].main.temp);
          }
        }
      });
    });
